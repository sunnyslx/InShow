...
public class OpenGlUtils {
	...
	public static int loadTexture(Context context,int resourceId){
		...
		//设置每个过滤器
		//GL_TEXTURE_MIN_FILTER：缩小的情况
		//GL_LINEAR_MIPMAP_LINEAR:使用三线性过滤
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_LINEAR);
		//GL_TEXTURE_MAG_FILTER：放大的情况
		//GL_LINEAR:双线性过滤
		//放大只能用最临近过滤（GL_NEAREST）和双线性过滤
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);

		//告诉OpenGL读入bitmap定义的位图数据，并把它复制到当前绑定的纹理对象
		texImage2D(GL_TEXTURE_2D,0,bitmap,0);
		//不需要Android位图了，释放位图数据
		bitmap.recycle();
	...
	public static String readShaderFromRawResource(final int resourceId){
		final InputStream inputStream =InShowParams.context.getResources().openRawResource(
				resourceId);
