...
{ 
	...
	vec4 c3 = normal(c1, lighten(c1,c2), 0.6); // lighten 点亮(0.6)
    c3 = adjust(c3, 0.12, 0., 0.05); // brightness 亮度 = 12, saturation 饱和度 = 0.5;
    c3 = vibrance(level(c3, curve), 0.5); // vibrance 振动= 0.5;
	c3 = normal(c3, overlay(c3, vec4(0.)), 1.-texture2D(samplerMask, textureCoordinate).r); // vignetting 晕影
